...
module.exports =
{
  ...
  apache: [
    ...
    { format: '%{timestamp}<[0-9]+>,%h,%b,%<"?>%{ress-name}<[a-zA-Z0-9\\+\\&\\- :,.\'éèàç]+>%<"?>,%U,%>s,%<"?>%{mt}<[a-zA-Z0-9\\+\\-\\/;= .]+>%<"?>,%<"?>%{profil1}<[a-zA-Z0-9/ \-_éèàç]*>%<"?>,%{uid}<[a-zA-Z0-9]*>,%{session-id}<[a-zA-Z0-9]*>,%<"?>%{profil2}<[a-zA-Z0-9/ \-_éèàç]*>%<"?>' } //Bibliopam EC-Lyon
  ],
  squid: [
